...
class Ion_auth
{
	...
	public function register_seller($identity, $password, $email, $additional_data = [], $group_ids = [])
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('email_activation', 'ion_auth');

		$id = $this->ion_auth_model->register_seller($identity, $password, $email, $additional_data, $group_ids);
		...
		else
		{
			...
			$activation_code = $this->ion_auth_model->activation_code;
			$identity        = $this->config->item('identity', 'ion_auth');
			$business        = $this->general_model->getOne('sellers',array('id' => $id));
	...
	public function seller_logged_in()
	{
		$this->ion_auth_model->trigger_events('logged_in');

		$recheck = $this->ion_auth_model->recheck_seller_session();

		// auto-login the user if they are remembered
		if (!$recheck && get_cookie($this->config->item('remember_cookie_name', 'ion_auth')))
		{
			$recheck = $this->ion_auth_model->login_remembered_seller();
		}
	...
	public function seller_forgotten_password($identity)
	{
		...
		$business = $this->ion_auth_model->seller($identity);
		
		if ($business)
		{
			// Generate code
			$code = $this->ion_auth_model->seller_forgotten_password($identity);
	...
	public function seller_forgotten_password_check($code)
	{
		$business = $this->ion_auth_model->get_seller_by_forgotten_password_code($code);
		...
		else
		{
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0)
			{
				//Make sure it isn't expired
				$expiration = $this->config->item('forgot_password_expiration', 'ion_auth');
				if (time() - $business->forgotten_password_time > $expiration)
				{
					//it has expired
					$identity = $business->{$this->config->item('identity', 'ion_auth')};
					$this->ion_auth_model->clear_seller_forgotten_password_code($identity);
					$this->set_error('password_change_unsuccessful');
					return FALSE;
				}
			}
	...
	public function seller_logout()
	{
		...
		$this->session->unset_userdata([$identity, 'id', 'business_id']);
