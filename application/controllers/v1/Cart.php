...
class Cart extends REST_Controller {
    ...
    public function add_to_cart_post() {
        ...
        $this->form_validation->set_rules('quantity', '', 'required', array('required' => '%s'));

      if ($this->form_validation->run() == true) {
         $post_data=array('product_id'=>$this->input->post('product_id'),
                         'quantity'=>$this->input->post('quantity'),
                         'user_id'=>$authorized_user['account']->id);
        $data = $this->cart_model->add_to_cart($post_data);
        if ($data['status']==1) {
            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('cart_item_add_success')
            ], REST_Controller::HTTP_OK);
        } else if($data['status']==2){
            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('cart_item_already')
            ], REST_Controller::HTTP_OK);
        }else{
             $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('cart_item_add_failed')
            ], REST_Controller::HTTP_OK);
        }
    ...
    public function cart_in_item_post() {
        /* Check Authentications */
        $headers = $this->input->request_headers();
        ...
        $data = $this->cart_model->get_cart_item($post_data);

        //print_r($data); die;
        if($data){
            $promo_res = $this->general_model->getOne('ecom_promocode_usage_history',$post_data);
            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('cart_item_found_success'),
                'total_quantity'=>$data['total_quantity'],
                'total_amount'=>$data['total_amount'],
                'promocode_data'=>$promo_res,
                 $this->config->item('rest_data_field_name')     => $data['data']
            ], REST_Controller::HTTP_OK);
        }else{
             $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
...
public function delete_cart_item_post() {
        /* Check Authentications */
        $headers = $this->input->request_headers();
        ...
        /* End Check Authentications */
    $this->form_validation->set_rules('cart_item_id', '', 'required', array('required' => '%s'));
    if($this->form_validation->run() == true) {
            
        $post_data=array('user_id'=>$authorized_user['account']->id,
                         'cart_item_id'=>$this->input->post('cart_item_id'),
                         'cart_id'=>$this->input->post('cart_id'));
        $data = $this->cart_model->delete_cart_item($post_data);
        if($data){
            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('cart_item_delete_success')
            ], REST_Controller::HTTP_OK);
        }else{
             $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('cart_item_delete_failed')
            ], REST_Controller::HTTP_OK);
        }
     }else{
            $this->response([
    ...
    if($this->form_validation->run() == true) {
        ...
        $post_data=array('cart_item_id'=>$this->input->post('cart_item_id'),
                         'quantity'=>$this->input->post('quantity'));
        $data = $this->cart_model->update_cart_item($post_data);
        if($data){
            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('cart_item_update_success')
            ], REST_Controller::HTTP_OK);
        }else{
             $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('cart_item_update_failed')
            ], REST_Controller::HTTP_OK);
        }
     }else{
            $this->response([
    ...
    public function get_checkout_details_post() {
        /* Check Authentications */
        $headers = $this->input->request_headers();
        ...
        $post_data=array('user_id'=>$authorized_user['account']->id);
        $data = $this->cart_model->get_checkout_details($post_data);

        //print_r($data); die;
        if($data){
              $delivery_charge = $this->cart_model->get_sql_query('ecom_delivery_charges','amount',array('status'=>'1'));

            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('checkout_found_success'),
                'total_quantity'=>$data['total_quantity'],
                'total_amount'=>$data['total_amount'],
                'delivery_charge'=>$delivery_charge[0]->amount,
                'final_amount'=>$data['total_amount']+$delivery_charge[0]->amount
                 //$this->config->item('rest_data_field_name')     => $data['data']
            ], REST_Controller::HTTP_OK);
        }else{
             $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('checkout_found_failed')
            ], REST_Controller::HTTP_OK);
        }
    }
}
