...
class Account extends REST_Controller {
            ...
            if ($login['status'] == 1) {
                ...
                $data = [
                 ...
                 $this->response([
                    ...
                    $this->config->item('rest_message_field_name')  =>'OTP Send Successfully ',
    ...
    public function social_signup_post() {
        ...
        if ($this->form_validation->run() == true) {
                    ...
                    $input_arr=array('email'=> $this->input->post('email'),
                                     'phone'=>$this->input->post('phone'),
                                     'password'=>$this->input->post('password'),
                                     'device_token'=>$this->input->post('device_token'),
                                     'type'=>$this->input->post('type'),
                                     'username'=>$this->input->post('username'));
                    $register = $this->ion_auth->social_register($input_arr);
                   
                    if ($register['success'] == 1) {
                        if($this->input->post('type')==1)
                          {
                             $this->response([
                            $this->config->item('rest_status_field_name') => $register['success'],
                            $this->config->item('rest_message_field_name') => $register['message'],
                            'data' => [
                                 'otp'        => $register['otp']]
                                ], REST_Controller::HTTP_OK); 
                          }else{
    ...
    */
    public function get_profile_get()
    {
        ...
        $data = [
            'user_id'           => (int) $authorized_user['account']->id,
            'email'             => !empty($authorized_user['account']->email) ? $authorized_user['account']->email : "",
            'first_name'         => !empty($authorized_user['account']->first_name) ? $authorized_user['account']->first_name : "",
             'username'         => !empty($authorized_user['account']->username) ? $authorized_user['account']->username : "",
            'country_code'      => !empty($authorized_user['account']->country_code) ? $authorized_user['account']->country_code : "",
 ...
 /**
         ...
         public function otp_verify_post()
    {
        ...
        if ($this->form_validation->run() == true) {
           ...
           $phone  = $this->input->post('phone');
           $authorized_user['account'] = $this->general_model->getOne('users', ['otp'=>$otp,'phone'=>$phone]);
...
/**
    ...
    public function add_address_post()
    {
        ...
        $this->form_validation->set_rules('full_name', '', 'required', array('required' => '%s'));
           ...
           $this->form_validation->set_rules('postcode', '', 'required', array('required' => '%s'));
    
        if ($this->form_validation->run() == true) {
            $check_available = $this->general_model->getOne('ecom_addresses', ['address' =>$this->input->post('address')]);
            if(!empty($check_available))
            {
                $this->response([
                    $this->config->item('rest_status_field_name')       => $this->config->item('rest_status_code_zero'),
                    $this->config->item('rest_message_field_name')      => $this->lang->line('user_address_already_add')
                ], REST_Controller::HTTP_OK);
            }
           // print_r($check_available); die;
            $data=array('user_id'=>$authorized_user['account']->id,
                        ...
                        'default_status'=>$this->input->post('default'));
            if($this->input->post('default')=='1')
            {

            $this->general_model->update('ecom_addresses',array('user_id'=>$authorized_user['account']->id),array('default_status'=>'0'));
            }
            $res = $this->general_model->insert('ecom_addresses',$data);
            if (!empty($res)) {
                    $this->response([
                        $this->config->item('rest_status_field_name')       => $this->config->item('rest_status_code_one'),
                        $this->config->item('rest_message_field_name')      =>$this->lang->line('user_address_add_success')
                    ], REST_Controller::HTTP_OK);
              
            } else {
                $this->response([
                    $this->config->item('rest_status_field_name')       => $this->config->item('rest_status_code_zero'),
                    $this->config->item('rest_message_field_name')      => $this->lang->line('user_address_add_failed')
                ], REST_Controller::HTTP_OK);
            }
        } else {
          $this->response([
  ...
  public function address_list_post() {
        ...
        $data = $this->general_model->getAll('ecom_addresses',$post_data);
        if($data){
            $address_default_res = $this->general_model->getAll('ecom_addresses',array('user_id'=>$authorized_user['account']->id,'default_status'=>'1'));
            if(!empty($address_default_res))
            {
                 $address_default_set_status=1;
            }else{
                $address_default_set_status=0;
            }
            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('user_address_found_success'),
                'address_default_set_status'=>$address_default_set_status,
                 $this->config->item('rest_data_field_name')     => $data
            ], REST_Controller::HTTP_OK);
        }else{
             $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('user_address_found_failed')
            ], REST_Controller::HTTP_OK);
        }
    }
...
public function delete_address_post() {
        ...
        /* End Check Authentications */
    $this->form_validation->set_rules('address_id', '', 'required', array('required' => '%s'));
    if($this->form_validation->run() == true) {
            
        $post_data=array('user_id'=>$authorized_user['account']->id,
                         'id'=>$this->input->post('address_id'));
        $data = $this->general_model->delete('ecom_addresses',$post_data);
        if($data){
            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('user_address_delete_success')
            ], REST_Controller::HTTP_OK);
        }else{
             $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('user_address_delete_failed')
            ], REST_Controller::HTTP_OK);
        }
     }else{
            $this->response([
    ...
    if($this->form_validation->run() == true) {
         $data=array('first_name'=>$this->input->post('full_name'),
                        'address'=>$this->input->post('address'),
                        'state'=>$this->input->post('state'),
                        'city'=>$this->input->post('city'),
                        'postcode'=>$this->input->post('postcode'),
         ...
         $where=array('user_id'=>$authorized_user['account']->id,
                         'id'=>$this->input->post('address_id'));
        $res = $this->general_model->update('ecom_addresses',$where,$data);
        if($res){
            $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('user_address_update_success')
            ], REST_Controller::HTTP_OK);
        }else{
             $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('user_address_update_failed')
            ], REST_Controller::HTTP_OK);
        }
     }else{
            $this->response([
        ...
        public function default_address_set_post()
    {
        /* Check Authentications */
        $headers = $this->input->request_headers();
        ...
        /* End Check Authentications */
    $this->form_validation->set_rules('address_id', '', 'required', array('required' => '%s'));
    $this->form_validation->set_rules('default', '', 'required', array('required' => '%s'));
    ...
    if($this->form_validation->run() == true) {
        ...
        if($this->input->post('default')=='1')
            ...
            $this->general_model->update('ecom_addresses',array('user_id'=>$authorized_user['account']->id,'id'=>$this->input->post('address_id')),array('default_status'=>'1'));

            $post_data=array('user_id'=>$authorized_user['account']->id,'default_status'=>'1');
            $data = $this->general_model->getAll('ecom_addresses',$post_data);

              $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_one'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('user_default_address_set_success'),
                  $this->config->item('rest_data_field_name')     => $data
            ], REST_Controller::HTTP_OK);
            }else{
                $this->response([
                $this->config->item('rest_status_field_name')   => $this->config->item('rest_status_code_zero'),
                $this->config->item('rest_message_field_name')  => $this->lang->line('user_default_address_set_failed')
            ], REST_Controller::HTTP_OK);
            }
    }else{
            $this->response([
