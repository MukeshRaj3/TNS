...
class Cities extends Seller_Controller {

  public function __construct() {
      parent::__construct();

      /* Load :: Common */
      $this->load->model('seller/cities_model');
  }
  ...
  public function index() {
    ...
    /* Load Template */
    $this->template->seller_render('seller/cities/index', $this->data);
  }
  ...
  public function create() {
    ...
    if ($this->form_validation->run() == TRUE) {
      ...
      if ($check) {
        ...
        redirect('seller/cities/create', 'refresh');
      }
      ...
      /* Load Template */
      $this->template->seller_render('seller/cities/add', $this->data);
    }
  }
  ...
  public function edit($id = NULL) {
      if (is_null($id)) {
        ...
        redirect('seller/cities', 'refresh');	
      }
    ...
    if ($this->form_validation->run() == TRUE) {
      ...
      if (!empty($check)) {
        if ($check->id != $id) {
          $this->session->set_flashdata('message', ['0', 'This city is already added']);
          redirect('seller/cities/edit/'.$id, 'refresh');
        }
      }
      $insert = [
        'name' => $this->input->post('name'),
      ...
      /* Load Template */
      $this->template->seller_render('seller/cities/edit', $this->data);
    }
  }
