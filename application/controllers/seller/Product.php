       ...
       /* Validate form input */
            ...
            if ($this->form_validation->run() == true) {    
              ...
              $this->data['categories'] = $this->product_model->get_category();
                ...
                $this->template->seller_render_page('seller/product/add_product',$this->data);
            }
    ...
    public function edit_product($id) {
       /* Validate form input */
            ...
            if ($this->form_validation->run() == true) {    
                ...
                /* Register new business  */
                $update_id = $this->product_model->update($id,$additional_data);
                if ($update_id) {
                        /*-----------------profile_pic----------------------*/  
                   if(!empty(($_FILES['image']['name'])))
                     {
                       $this->general_model->delete('ecom_product_images',array('pid'=>$id));
                       $product_img_path='./image/product/';
                       $new_file_name=$this->product_model->file_upload('pimg',$profile_pic_path);
                       $product_image = $new_file_name;
                       $this->product_model->img_insert(array('search_image'=>$product_image,'cart_image'=>$product_image,'catalog_image'=>$product_image,'pid'=>$insert_id));
                     }
                $this->session->set_flashdata('message', ['1','Product edit successfully.']);
                            redirect('seller/product', 'refresh');
                } else {
                    $this->session->set_flashdata('message', ['0','Product edit failed.']);
                    redirect('seller/product/edit_product/'.$id, 'refresh');
                }
            } else {
                $this->data['res']=$this->product_model->get_product($id)[0];
                $this->data['product_img']=$this->general_model->getAll('ecom_product_images',array('pid'=>$id));
                $this->data['categories'] = $this->product_model->get_category($this->session->userdata('user_id'));

                //print_r($this->data['res']); die;
                $this->template->seller_render_page('seller/product/edit_product',$this->data);
            }
