...
class Party_model extends CI_Model {
    ...
    public function get_organization_by_id_09_02_2023($organization_id) {
     ...
     public function get_organization_by_id($organization_id) {
        $res= $this->db->get_where($this->tables['organization'], ['id' => $organization_id])->row();
        if(!empty($res))
        {
            $sql=$this->db->query('SELECT * FROM organization_amenities WHERE id IN('.$res->org_amenitie_id.')');
            $res_oimm=$sql->result_array();
            if(!empty($res_oimm))
            {
                $res->organization_amenities=$res_oimm;
            }else{
                 $res->organization_amenities=$res_oimm;  
            }
...
public function party_amenities() {
     ...
     if ($query->num_rows()>0) {
      ...
      public function get_organisation($name) {
        return $this->db->get_where($this->tables['organization'], ['name' => $name])->num_rows();
    }
