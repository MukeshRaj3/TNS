...
class Ion_auth_model extends CI_Model
{
    ...
    public function social_register($data) {
    	 //print_r($data);  die;
    	  if($data['type']==1)
    	  {
               $query_phone = $this->db->select('users.email,
			users.id')
		->from('users')
        ->where('role_id',2)
        ...
        ->order_by('id', 'desc')
        ->get();
        if ($query_phone->num_rows()===1) {
                $user = $query_phone->row();
        	    $opt=123456; //get_otp();
    		...
    		$query_email = $this->db->select('users.email,
			users.id')
		->from('users')
        ->where('role_id',2)
...
public function seller_recheck_session()
	{
		$recheck = (NULL !== $this->config->item('recheck_timer', 'ion_auth')) ? $this->config->item('recheck_timer', 'ion_auth') : 0;
	...
	public function login_remembered_user()
	{
		...
		public function seller_login_remembered_user()
	{
		$this->trigger_events('pre_login_remembered_user');
	...
	public function phone_login_api($phone, $remember = FALSE, $code = false) {
        ...
        if ($query->num_rows() === 1) {
        	$user = $query->row();
        		...
        		$this->trigger_events(['post_login', 'post_login_successful']);
        		$this->set_message('login_successful');
                $user =  $this->db->get_where($this->tables['users'], ['id' =>$user->id])->row();
			...
			$response = array('status' =>0, 'message' => $this->lang->line('login_phone_account_not_found'));
			return $response;
		}
