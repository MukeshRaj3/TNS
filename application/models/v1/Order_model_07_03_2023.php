...
public function get_my_orders($user_id,$filter_type) {
         $this->db->select($this->tables['ecom_orders'].'.id as order_id,order_no,order_datetime,delivered,cancelled,'.'sum('.$this->tables['ecom_order_items'].'.quantity) as total_qty,'.'sum('.$this->tables['ecom_order_items'].'.unit_price) as total_amount');
        ...
        if($filter_type==1)
        {
        $this->db->where($this->tables['ecom_orders'].'.delivered','0');

        }
        if($filter_type==2)
        {
        $this->db->where($this->tables['ecom_orders'].'.delivered','1');

        }
         if($filter_type==3)
        {
        $this->db->where($this->tables['ecom_orders'].'.cancelled','1');
        ...
        if($query->num_rows() > 0) 
        {
            ...
            foreach ($res as $key => $value) {
               ...
               {
                $res[$key]->delivered='Proccessing';
               }else if($value->delivered=='1')
               {
                $res[$key]->delivered='Delivered';
               }else{
                 $res[$key]->delivered='Pending'; 
               }
                  if($value->cancelled=='1')
               {
                $res[$key]->cancelled='Cancelled';
               }else{
                 $res[$key]->cancelled=''; 
               }
            }
    ...
    public function get_order_details($user_id,$order_id) {
         $this->db->select($this->tables['ecom_orders'].'.id as order_id,order_no,order_datetime,delivered,cancelled,delivery_charges,payment_status,'.$this->tables['ecom_order_items'].'.pid,quantity,unit_price,item_price,'.$this->tables['ecom_addresses'].'.first_name,last_name,address,city,state,country,postcode,phone,email,type');
         $this->db->join($this->tables['ecom_order_items'], $this->tables['ecom_orders'].'.id = '.$this->tables['ecom_order_items'].'.order_id', 'LEFT');
        ...
        if($query->num_rows() > 0) 
        {
            ...
            $res[0]->order_date=date('d-m-Y',strtotime($res[0]->order_datetime));
               if($res[0]->delivered=='0')
               {
                $res[0]->delivered='Proccessing';
               }else if($res[0]->delivered=='1')
               {
                $res[0]->delivered='Delivered';
               }else{
                 $res[0]->delivered='Pending'; 
               }
                if($res[0]->cancelled=='1')
               {
                $res[0]->cancelled='Cancelled';
               }else{
                 $res[0]->cancelled=''; 
               }
          ...
          $arr=array('order_id'=>$res[0]->order_id,'order_no'=>$res[0]->order_no,'order_date'=>$res[0]->order_date,'delivered'=>$res[0]->delivered,'cancelled'=>$res[0]->cancelled,'delivery_charges'=>$res[0]->delivery_charges,'payment_status'=>$res[0]->payment_status,'full_name'=>$res[0]->first_name,'address'=>$res[0]->address,'city'=>$res[0]->city,'state'=>$res[0]->state,'country'=>$res[0]->country,'postcode'=>$res[0]->postcode,'phone'=>$res[0]->phone,'email'=>$res[0]->email,'type'=>$res[0]->type,'item_details'=>$product_arr);
           return $arr;
        }else{
             return false;
         }
